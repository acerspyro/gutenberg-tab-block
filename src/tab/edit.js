...
export default (props) => {
	const { attributes, setAttributes, setBackgroundColor, toggleSelection } =
		props;
	...
	return (
		<Fragment>
				...
				<PanelBody title={__("Background image")} initialOpen={false}>
					...
					{!!bgImage && (
						<div className="tab-bg-settings">
							...
							{!bgOptions.fixed && !bgOptions.stretch && (
								<ToggleControl
			...
			>
				...
				{!!enableSpacing && (
					<Resizable
						className={classnames("spacing-box", {
							resizing: resizeTopIsActive,
						})}
						size={{
						...
						handleClasses={{
							...
							bottom:
								"core-blocks-spacer__resize-handler-bottom components-resizable-box__handle",
						}}
						enable={{
							top: false,
							right: false,
				...
				{!!enableSpacing && (
					<Resizable
						...
						onResizeStop={(event, direction, elt, delta) => {
							setAttributes({
								spacingBottom: parseInt(
									attributes.spacingBottom + delta.height,
									10
								),
